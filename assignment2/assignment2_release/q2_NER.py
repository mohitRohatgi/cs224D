...
class NERModel(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ...
      with tf.variable_scope("embedding_layer") as scope:
        ...
        window = tf.reshape(window, shape=[-1, self.config.window_size * 
                                    self.config.embed_size], name="window")
  ...
  def add_loss_op(self, y):
    ...
    loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=y, 
                                            logits=self.labels_placeholder))
...
def test_NER():
  ...
  with tf.Graph().as_default():
    ...
    with tf.Session() as session:
      ...
      print('Test')
      print('=-=-=')
      print('Writing predictions to q2_test.txt')
      _, predictions = model.predict(session, model.X_test, model.y_test)
      save_predictions(predictions, "q2_test.txt")
