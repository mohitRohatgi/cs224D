...
class RNNLM_Model(LanguageModel):
  ...
  def __init__(self, config):
    ...
    self.calculate_loss = self.add_loss_op(tf.stack(self.outputs))
    ...
    with tf.device('/cpu:0'):
      ...
      with tf.variable_scope("embedding") as scope:
          ...
          for i, _input in enumerate(inputs):
              inputs[i] = tf.squeeze(_input, axis=[1])
      ### END YOUR CODE
      return inputs
  ...
  def add_loss_op(self, output):
    ...
    """
    ### YOUR CODE HERE
    target = tf.reshape(self.labels_placeholder, shape=(self.config.num_steps, -1))
    target = tf.cast(target, tf.int32)
    all_ones = tf.ones((self.config.num_steps, self.config.batch_size))
    cross_entropy = sequence_loss(output, target, all_ones, len(self.vocab))
