...
def negSamplingCostAndGradient(predicted, target, outputVectors, dataset, 
    ...
    probs = softmax(vecs.dot(predicted))
    cost = -np.sum(np.log(probs * labels))
    
    dx = probs
    dx[0] -= 1
    gradPred = dx.reshape((1, K+1)).dot(vecs).flatten()
    grad[indices] = dx.reshape((K+1, 1)).dot(predicted.reshape(1, predicted.shape[0]))  
...
def skipgram(currentWord, C, contextWords, tokens, inputVectors, outputVectors, 
    ...
    for j in contextWords:
        target = tokens[j]
        c_cost, c_gradPred, c_grad = word2vecCostAndGradient(predicted, target,
...
def cbow(currentWord, C, contextWords, tokens, inputVectors, outputVectors, 
    ...
    predicted = np.sum(d, axis=0) / 2.0 / C

    cost, gradPred, gradOut = word2vecCostAndGradient(predicted, c_idx,
                                                      outputVectors, dataset)

    gradIn = np.zeros(inputVectors.shape)
    for word in contextWords:
        gradIn[tokens[word]] += gradPred / 2.0 / C
    ### END YOUR CODE
